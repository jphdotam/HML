...
MODEL_DIR = "./output/models/densenets"
TEST_FOLD = 4


if __name__ == "__main__":
    cfg_all, model_dir, vis_dir, log_dir = load_config(CONFIG)
    arch_names, arch_accus, arch_kappas, arch_params = [], [], [], []

    for arch in cfg_all['training']['arch']:
        cfg = cfg_all.copy()
        cfg['training']['arch'] = arch
        state_paths = glob(os.path.join(MODEL_DIR, f"{arch}_*.pt"))
        ...
        # Test
        targets, predicted_classes, filenames, kappa, accuracy, cm, n_params = test(state_paths, dl_test, cfg, return_n_params=True)
        ...
        arch_accus.append(round(accuracy, 3))
        arch_kappas.append(round(kappa, 3))
        arch_params.append(n_params)

    df = pd.DataFrame.from_dict({'arch': arch_names, 'params': arch_params, 'accu': arch_accus, 'kappa': arch_kappas})

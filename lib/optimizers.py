...
def load_optimizer(model, cfg, state, steps_per_epoch=None):
    ...
    if opt_path := state.get('optimizer', None):
    ...
    # Scheduler found in state dict
    if scheduler:
        if not sched:
            print(f"WARNING Optimizer present in state dict but not using as per config!")
    # No scheduler in state dict
    else:
        scheduler = None
